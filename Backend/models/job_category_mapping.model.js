...
job_category_mapping.getById = (id, callback) => {
  const sqlString = "SELECT * FROM job_category_mapping WHERE job_id = ? ";
...
job_category_mapping.update = (job_category_mapping, id, callBack) => {
  const sqlString = "UPDATE job_category_mapping SET ? WHERE job_id = ?";
...
job_category_mapping.delete = (id, callBack) => {
  db.query(`DELETE FROM job_category_mapping WHERE job_id = ?`, id, (err, res) => {
    if (err) {
      callBack(err);
      return;
    }
